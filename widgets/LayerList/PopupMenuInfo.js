...
define([
  ...
  var clazz = declare([], {
    ...
    constructor: function (
      ...
      if (layerType === "ArcGISDynamicMapServiceLayer") {
        displayItemInfos.unshift({ key: "separator" });
        displayItemInfos.unshift({
          key: "remove",
        });
      }
    ...
    _getATagLabel: function () {
      ...
      return (
        '<a class="menu-item-description" target="_blank" href="' +
        url +
        '">' +
        label +
        "</a>"
    ...
    _getItemDetailsPageUrl: function (basicItemInfo) {
      var itemUrl = "";
      itemUrl = portalUrlUtils.getItemDetailsPageUrl(
        basicItemInfo.portalUrl,
        basicItemInfo.itemId
      );
      return itemUrl;
    },
    ...
    */
    ...
    getDeniedItems: function () {
      ...
      if (
        this.layerListWidget.layerListView.isFirstDisplayedLayerInfo(
          this._layerInfo
        )
      ) {
      ...
      if (
        this.layerListWidget.layerListView.isLastDisplayedLayerInfo(
          this._layerInfo
        )
      ) {
        dynamicDeniedItems.push({
      ...
      if (this._layerInfo.originOperLayer.featureCollection) {
        ...
        !this._layerInfo.isRootLayer() &&
        this._layerInfo.getRootLayerInfo().layerObject.declaredClass ===
      ...
      var getSupportTableInfoDef = this._getSupportTableInfoForAllSublayers(
        this._layerInfo
      );
      ...
      all({
        ...
        lang.hitch(this, function (result) {
          ...
          var attributeTableWidget =
            this.layerListWidget.appConfig.getConfigElementsByName(
              "AttributeTable"
            )[0];
          ...
          if (!attributeTableWidget || !attributeTableWidget.visible) {
            ...
            !this._isSupportedByAT(attributeTableWidget, supportTableInfo)
          ) {
            dynamicDeniedItems.push({
              key: "table",
              denyType: "disable",
    ...
    _isValidExtent: function (extent) {
      ...
      if (esriLang.isDefined(extent)) {
        if (
          esriLang.isDefined(extent.xmin) &&
          isFinite(extent.xmin) &&
          esriLang.isDefined(extent.ymin) &&
          isFinite(extent.ymin) &&
          esriLang.isDefined(extent.xmax) &&
          isFinite(extent.xmax) &&
          esriLang.isDefined(extent.ymax) &&
          isFinite(extent.ymax)
        ) {
          isValid = true;
        }
      }
    ...
    _deleteLayer: function (e) {
      var map = e.layerListView._lastLayerInfo.map;
      let serviceName =
        e.layerListView._currentSelectedLayerRowNode.ariaLabel.split(" ")[1];
      console.log(serviceName);
      let layerId;
      map.layerIds.forEach((id) => {
        if (map.getLayer(id).url.includes(serviceName)) {
          layerId = id;
        }
      });
    ...
    _onTableItemClick: function (evt) {
      this._getSupportTableInfoForAllSublayers(this._layerInfo).then(
        lang.hitch(this, function (allSupportTableInfo) {
          var widgetManager;
          ...
          if (
            this._isSupportedByAT(attributeTableWidgetEle, allSupportTableInfo)
          ) {
            ...
            array.forEach(
              ...
              function (supportTableInfo) {
                if (supportTableInfo.isSupportedLayer) {
                  widgetManager
                    .triggerWidgetOpen(attributeTableWidgetEle.id)
                    .then(
                      lang.hitch(this, function (atWidget) {
                        if (atWidget) {
  ...
  clazz.create = function (layerInfo, layerListWidget) {
    ...
    layerInfo.getLayerType().then(
      lang.hitch(this, function (layerType) {
        ...
        if (layerInfo.isBasemap() && layerInfo.isRootLayer()) {
          ...
          layerInfo.getRootLayerInfo().layerObject.declaredClass ===
            "esri.layers.ArcGISDynamicMapServiceLayer"
        ) {
