  ...
  return declare([DropMenu, _TemplatedMixin], {
    ...
    _initDeniedItems: function () {
      ...
      for (var menuItem in this._config.contextMenu) {
        if (
          this._config.contextMenu.hasOwnProperty(menuItem) &&
          typeof this._config.contextMenu[menuItem] !== "function" &&
          this._config.contextMenu[menuItem] === false
        ) {
          deniedItemsFromConfigKeys = deniedItemsFromConfigKeys.concat(
            menuItemDictionary[menuItem].split(" ")
          );
        }
      }
    ...
    _denyItems: function () {
          ...
          if (itemNode) {
            if (deniedItem.denyType === "disable") {
              ...
              if (deniedItem.key === "url") {
                query(".menu-item-description", itemNode).forEach(function (
                  itemA
                ) {
      ...
      if (displayItemNodes.length === 0) {
        ...
        if (
          html.hasClass(
            displayItemNodes[displayItemNodes.length - 1],
            "menu-item-line"
          )
        ) {
          html.addClass(
    ...
    openDropMenu: function () {
      ...
      if (!this.dropMenuNode) {
        ...
        PopupMenuInfo.create(this._layerInfo, this.layerListWidget).then(
          lang.hitch(this, function (popupMenuInfo) {
      ...
      popupMenuInfoDef.then(
        lang.hitch(this, function () {
          // get deniedItems
          this.popupMenuInfo.getDeniedItems().then(
            lang.hitch(this, function (deniedItems) {
              this._deniedItems =
                this._deniedItemsFromConfig.concat(deniedItems);
              // deny items
              this._refresh();
    ...
    showTransNode: function (transValue, item) {
      ...
      domStyle.set(
        this.transparencyDiv,
        "top",
        this._getTransNodePosition().top
      );
      if (isRTL) {
        domStyle.set(
          this.transparencyDiv,
          "left",
          this._getTransNodePosition().right
        );
      } else {
        domStyle.set(
          this.transparencyDiv,
    ...
    _createTransparencyWidget: function (item) {
      ...
      this.own(
        this.transHorizSlider.on(
          "change",
          lang.hitch(this, function (newTransValue) {
            var data = {
    ...
    showSetVisibilityRangeNode: function (layerInfo, item) {
      ...
      this.own(
        this.visibleScaleRangeSlider.on(
          "scale-range-change",
          lang.hitch(this, function (scaleRange) {
    ...
    */
    ...
    _supportsVisibilityRange508Accessibility: function (item) {
      var sliderBtns = query(
        ".dijitSliderImageHandle",
        this.setVisibilityRangeNode
      );
      var firstFocusNode = sliderBtns[0];
      var downArrowButtons = query(
      ...
      if (esriScaleMenuPopup) {
        ...
        if (esriHeaders[0] && esriCurrent) {
          html.setAttr(
            dijitInputInner,
            "aria-label",
            esriHeaders[0].innerText + " " + esriCurrent.innerText
          );
        }
          ...
          */
            ...
            this.own(
              on(
                ...
                "keydown",
                lang.hitch(this, function (e) {
                  var menuItemInfo = this._getMenuItemInfo(
                    esriScaleMenuPopup,
                    menuItem
                  );
    ...
    _getMenuItemInfo: function (esriScaleMenuPopup, currentMenuItem) {
      ...
      var esriScaleMenuItems = query("li.esriItem", esriScaleMenuPopup);
      esriScaleMenuItems = array.filter(
        esriScaleMenuItems,
        lang.hitch(this, function (menuItem) {
          if (html.hasClass(menuItem, "esriHidden")) {
            return false;
          } else {
            return true;
      ...
      menuItemInfo.lastMenuItem =
        esriScaleMenuItems[esriScaleMenuItems.length - 1];
      return menuItemInfo;
    },
